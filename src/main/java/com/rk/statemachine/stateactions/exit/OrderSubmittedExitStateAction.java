package com.rk.statemachine.stateactions.exit;
...
@Service
@Slf4j
public class OrderSubmittedExitStateAction implements Action<OrderState, OrderEvent> {

    @Override
    public void execute(StateContext<OrderState, OrderEvent> context) {
        Long orderId = Long.class.cast(context.getExtendedState().getVariables().getOrDefault("orderId",-1L));
        log.info("Order Id : {}",orderId);
        log.info("exiting from submitted state!!");
    }
}
